    ...
    def run_tests_from_file(self, test_name):
        with open(path.join(WD, test_name)) as f:
            for line in f:
                ...
                expected_scores = expected_scores.replace("(", "").replace(")", "").replace("\n", "").split(", ")
                expected_score, expected_macro = float(expected_scores[0]), expected_scores[1]
                result = CVSS4(vector)
                actual_score = result.score()
                actual_macro = "".join(map(str, result.macro_vector()))

                self.assertEqual(expected_score, actual_score, test_name + " - " + vector)
                self.assertEqual(expected_macro, actual_macro, test_name + " - " + vector)

    def test_simple(self):
        """
        All vector combinations with only mandatory fields, 104,976 vectors.
        """
        self.run_tests_from_file("vectors_simple4")
